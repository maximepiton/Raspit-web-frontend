...
function initIt()
{
	...
	day.options[7] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
...
function newclick(E)
{
	...
	case "XBL":
			...
			if(archiveMode){
				var T_then = new Date(document.getElementById("archiveYear").value,
			                              document.getElementById("archiveMonth").value - 1,
			                              document.getElementById("archiveDay").value, 0, 0, 0, 0);
                        	var T = new Date();
				diff = T.getTime() - T_then.getTime();
				diff /= 24*60*60*1000;
				diff = 1 + Math.floor(diff);
				if(diff > SAVE_DAYS){
					alert("Only " + (SAVE_DAYS-1) + " past days available for X-section " +
					      "\nRequest is for " + (diff-1) + " days ago");
					return;
				}
			}
	...
	case "SkewT":
			if(archiveMode){
				...
				diff = 1 + Math.floor(diff);
				if(diff > SAVE_DAYS){
					alert("Only " + (SAVE_DAYS-1) + " past days available for SkewT " +
					      "\nRequest is for " + (diff-1) + " days ago");
					return;
				}
			}
